...
class Mangopay {
    ...
    constructor (username, password, opts) {
        ...
        assert(_.includes(_.values(_constants.API_VERSION), this.config.apiVersion), 'Unsupported API Version');
        ...
        this.Hook = {
            get: id => this._getHook(id),
            list: opts => this._listHooks(opts),
            update: (id, data) => this._updateHook(id, data),
            create: data => this._createHook(data)
        }
    }
    ...
    _bindUser (user) {
        ...
        user.update = data => this._updateUser(user.Id, user.LegalPersonType ? _constants.USER_TYPE.LEGAL : _constants.USER_TYPE.NATURAL, data);
    ...
    _bindHook (hook) {
        hook.reload = () => this._getHook(hook.Id);
        hook.update = data => this._updateHook(hook.Id, data);
    ...
    _createNaturalUser (_userData) {
        return validator(_userData, this._validationSchemas.createNaturalUser)
            .then(userData => this._createUser(_constants.USER_TYPE.NATURAL, userData));
    }

    _createLegalUser (_userData) {
        return this._createUser(_constants.USER_TYPE.LEGAL, _userData);
    }
    ...
    _updateNaturalUser (userId, _userData) {
        return validator(_userData, this._validationSchemas.updateNaturalUser)
            .then(userData => this._updateUser(userId, _constants.USER_TYPE.NATURAL, { body: userData }));
    }

    _updateLegalUser (userId, _userData) {
        return this._updateUser(userId, _constants.USER_TYPE.LEGAL, { body: _userData });
    }
    ...
    _getHook (notificationId) {
        return this._request('GET', `hooks/${ notificationId }`)
            .then(notification => this._bindHook(notification));
    }

    _listHooks (opts) {
        return this._request('GET', 'hooks', { qs: opts })
            .map(notification => this._bindHook(notification));
    }

    _createHook (notificationData) {
        return this._request('POST', 'hooks', { body: notificationData })
            .then(notification => this._bindHook(notification))
    }

    _updateHook (notificationId, notificationData) {
        return this._request('PUT', `hooks/${ notificationId }`, { body: notificationData })
            .then(notification => this._bindHook(notification));
    }
