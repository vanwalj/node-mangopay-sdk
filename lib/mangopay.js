...
class Mangopay {
    constructor (username, password, opts) {
        ...
        this.transfers = {
            get: this._getTransfers.bind(this),
            create: this._createTransfer.bind(this)
        };

        this.KYCs = {
            documents: {
                get: this._getKYCDocuments.bind(this)
            }
        };

        this.cards = {
            get: this._getCards.bind(this),
            update: this._updateCard.bind(this)
        };

        this.payOuts = {
            get: this._getPayOuts.bind(this),
            bankWire: {
        ...
        this.refunds = {
            get: this._getRefunds.bind(this)
        };

        this.preauthorizations = {
            get: this._getPreauthorizations,
            update: this._updatePreauthorization,
            card: {
                direct: this._createDirectCardPreauthorization.bind(this)
            }
        };

        this.disputes = {
            get: this._getDisputes.bind(this),
    ...
    _bindUser (user) {
        ...
        user.cards = {
            get: this._getUserCards.bind(this, user.id)
        };
        user.wallets = {
            get: this._getUserWallets.bind(this, user.id)
        };
        user.transactions = {
            get: this._getUserTransactions.bind(this, user.id)
        };
        user.bankAccounts = {
            get: this._getUserBankAccounts.bind(this, user.id),
            create: this._createUserBankAccount.bind(this, user.id)
        };
        user.KYCs = {
            documents: {
                get: this._getUserKYCDocuments.bind(this, user.id),
        ...
        user.reload = this._getUsers.bind(this, user.id);
    ...
    _bindWallet (wallet) {
        wallet.transactions = {
            get: this._getWalletTransactions.bind(this, wallet.id)
        };

        wallet.reload = this._getWallets.bind(this, wallet.id);
    ...
    _bindKycDocument (document, userId) {
        if (userId) {
            ...
            document.reload = this._getUserKYCDocuments.bind(this, userId, document.id);
            document.pages = {
                create: this._createUserKYCDocumentPage.bind(this, userId, document.id)
            };
        } else {
            document.reload = this._getKYCDocuments.bind(this, document.id);
        }
    ...
    _bindCardRegistration (cardRegistration) {
        ...
        cardRegistration.reload = this._getCardRegistrations.bind(this, cardRegistration.id);
    ...
    _bindCard (card) {
        ...
        card.reload = this._getCards.bind(this, card.id);

        return card;
    }

    _bindTransfer (transfer) {
        transfer.reload = this._getTransfers.bind(this, transfer.id);
        transfer.refund = {
    ...
    _bindBankAccount (bankAccount, userId) {
        bankAccount.reload = this._getUserBankAccounts.bind(this, userId, bankAccount.id);

        return bankAccount;
    }

    _bindPayOut (payOut) {
        payOut.reload = this._getPayOuts.bind(this, payOut.id);

        return payOut;
    }

    _bindPayIn (payIn) {
        payIn.reload = this._getPayIns.bind(this, payIn.id);
        payIn.refunds = {
    ...
    _bindRefund (refund) {
        refund.reload = this._getRefunds.bind(this, refund.id);

        return refund;
    }

    _bindDispute (dispute) {
        dispute.reload = this._getDisputes.bind(this, dispute.id);
    ...
    _bindPreauthorization (preauthorization) {
        preauthorization.reload = this._getPreauthorizations.bind(this, preauthorization.id);
    ...
    _getUsers (userId, page, perPage) {
        if (userId) {
            return this._request('GET', `/users/${ userId }`)
                .then(this._bindUser.bind(this));
        }
        return this._request('GET', '/users', { qs: { page: page, 'per_page': perPage } })
    ...
    _getUserCards (userId, page, perPage) {
        return this._request('GET', `/users/${ userId }/cards`, { qs: { page: page, 'per_page': perPage } })
            .map(this._bindCard.bind(this));
    }

    _getUserWallets (userId, page, perPage) {
        return this._request('GET', `/users/${ userId }/wallets`, { qs: { page: page, 'per_page': perPage } })
            .map(this._bindWallet.bind(this));
    }

    _getUserTransactions (userId, page, perPage) {
        return this._request('GET', `/users/${ userId }/transactions`, { qs: { page: page, 'per_page': perPage } });
    }

    _getUserBankAccounts (userId, bankAccountId, page, perPage) {
        if (bankAccountId) {
            return this._request('GET', `/users/${ userId }/bankaccounts/${ bankAccountId }`)
                .then(_.bind(this._bindBankAccount, this, _, userId));
        }
        return this._request('GET', `/users/${ userId }/bankaccounts`, { qs: { page: page, 'per_page': perPage } })
            .then(_.bind(this._bindBankAccount, this, _, userId));
    }
    ...
    _getWallets (walletId, page, perPage) {
        if (walletId) {
            return this._request('GET', `/wallets/${ walletId }`)
                .then(this._bindWallet.bind(this));
        }
        return this._request('GET', '/wallets', { qs: { page: page, 'per_page': perPage } })
            .map(this._bindWallet.bind(this));
    }
    ...
    _getWalletTransactions (walletId, page, perPage) {
        return this._request('GET', `/wallets/${ walletId }/transactions`, { qs: { page: page, 'per_page': perPage } });
    }

    _getPayIns (payInId, page, perPage) {
        if (payInId) {
            return this._request('GET', `/payins/${ payInId }`)
                .then(this._bindPayIn.bind(this));
        }
        return this._request('GET', '/payins', { qs: { page: page, 'per_page': perPage } })
            .then(this._bindPayIn.bind(this));
    }
    ...
    _getUserKYCDocuments (userId, documentId, page, perPage) {
        if (documentId) {
            return this._request('GET', `/users/${ userId }/KYC/documents/${ documentId }`)
                .then(_.bind(this._bindKycDocument, this, _, userId));
        }
        return this._request('GET', `/users/${ userId }/KYC/documents`, { qs: { page: page, 'per_page': perPage } })
    ...
    _getKYCDocuments (documentId, page, perPage) {
        if (documentId) {
            return this._request('GET', `/KYC/documents/${ documentId }`)
                .then(this._bindKycDocument.bind(this));
        }
        return this._request('GET', `/KYC/documents`, { qs: { page: page, 'per_page': perPage } })
    ...
    _getRefunds (refundId) {
        return this._request('GET', `/refunds/${ refundId }`)
            .then(this._bindRefund.bind(this));
    }

    _getPreauthorizations (preauthorizationId) {
    ...
    _getDisputes (disputeId, page, perPage) {
        if (disputeId) {
            return this._request('GET', `/disputes/${ disputeId }`)
                .then(this._bindDispute.bind(this));
        }
        return this._request('GET', '/disputes', { qs: { page: page, 'per_page': perPage } })
