...
const cardTypes = _.values(constants.CARD_TYPE);
...
Joi.cardType = () => Joi.string().valid(cardTypes);
Joi.IBAN = () => Joi.string().regex(/^[a-zA-Z]{2}\d{2}\s*(\w{4}\s*){2,7}\w{1,4}\s*$/);
...
Joi.cardRegistration = () => Joi.object().keys({
    Tag: Joi.string(),
    UserId: Joi.string(),
    Currency: Joi.currencyCode(),
    CardType: Joi.cardType(),
    CardRegistrationURL: Joi.string().uri(),
    RegistrationData: Joi.string()
});

Joi.createCardRegistration = () => Joi.cardRegistration().requiredKeys(
    'UserId', 'Currency'
);
...
module.exports['v2.01'] = {
    ...
    createCardRegistration: Joi.createCardRegistration(),
    updateCardRegistration: Joi.cardRegistration()
};